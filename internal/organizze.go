...
func GenerateOrganizzeXLXSSheet(month string, entries []model.Entry) error {
	f := excelize.NewFile()
	...
	_ = f.SetSheetName(f.GetSheetList()[0], month)
	_ = f.SetCellValue(month, "A1", "Data")
	_ = f.SetCellValue(month, "B1", "Descrição")
	_ = f.SetCellValue(month, "C1", "Categoria")
	_ = f.SetCellValue(month, "D1", "Valor")
	_ = f.SetCellValue(month, "E1", "Tipo")

	for i := 0; i < len(entries); i++ {
		row := i + 2

		_ = f.SetCellValue(month, fmt.Sprintf("A%d", row), entries[i].Date)
		_ = f.SetCellValue(month, fmt.Sprintf("B%d", row), entries[i].Description)
		_ = f.SetCellValue(month, fmt.Sprintf("C%d", row), entries[i].Category)
		_ = f.SetCellFloat(month, fmt.Sprintf("D%d", row), entries[i].Value, 2, 32)
		_ = f.SetCellValue(month, fmt.Sprintf("E%d", row), entries[i].Type)
	}
...
func GenerateOrganizzeCSV(entries []model.Entry) error {
    ...
    header := []string{"Date", "Description", "Category", "Value", "Type"}
